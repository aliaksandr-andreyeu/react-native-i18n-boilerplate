...
type TradingRecentActivityScreenProps = StackScreenProps<RootRootParamsList, ROOT_ROUTE_NAMES.TradingRecentActivity>;

interface TradingRecentActivityScreenData extends TradingRecentActivityScreenProps {
  dateRange: DateRange;
...
const TradingRecentActivityScreen: FC<TradingRecentActivityScreenData> = ({
  route,
  navigation,
  setOffset,
  setHistory,
  ...
  const _renderItem = useCallback(
    ({ item }: SectionListRenderItemInfo<{ item: HistoryDataItem[] }, HistoryData>) => {
                ...
                <BaseDealActivityCard
                  ...
                  onPress={() => {
                    navigation.navigate(ROOT_ROUTE_NAMES.PositionInfo, {
                      positionTicket: el.ticket as number,
                      positionId: el.positionId as number,
                      title: el.symbol as string,
                      isPosition: true,
                      isClosed: true,
                      account: accountId
                    });
    ...
    <>
      {isLoading ? (
        ...
        <Fragment>
          ...
          <SectionList
            contentContainerStyle={[{ paddingBottom: 34 + bottom }, history.length === 0 && { flex: 1 }]}
...
export default TradingRecentActivityScreen;
